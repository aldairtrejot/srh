...
function searchInit() {
    ...
    $.ajax({
        url: `${URL_DEFAULT}/filesdocument/table`,
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = `/srh/public/filesdocument/edit/${object.id_cat_documento}`;
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton1">
                                       ...
                                       <!-- Aquí se agrega la opción para eliminar -->
                                        <a class="dropdown-item" href="#" onclick="confirmDelete(${object.id_cat_documento})">
...
function paginatorMin1() {
    iterator = Math.max(iterator - 1, 0);
    setValue();
    searchInit();
}

function paginatorMin5() {
    iterator = Math.max(iterator - 5, 0);
    setValue();
    searchInit();
}
...
function setValue() {
    const current = iterator;              // usar 0-based real
    const previous = Math.max(current - 1, 0);
    const next = current + 1;

    document.getElementById("is_iterator").innerText = current;
    document.getElementById("is_iteratorMin").innerText = previous;
    document.getElementById("is_iteratorMax").innerText = next;
}
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/filesdocument/delete/${id}`,
        type: 'DELETE',
        data: { _token: token },
        success: () => {
