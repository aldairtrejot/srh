        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = `${URL_DEFAULT}/estatus/edit/${object.id}`;
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/estatus/delete/${id}`,
        type: 'DELETE',
        data: {
            _token: token
        },
        success: function(response) {
            window.location.href = '/srh/public/administration/estatusC/list';
        },
