...
function searchInit() {
    ...
    $.ajax({
        url: URL_DEFAULT.concat('/certification/table'),
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = URL_DEFAULT.concat(`/informative/edit/${object.id}`);
                        ...
                        <td class="button-column">
                            ...
                            ` : `
                                <div class="button-container">
                                    <button onclick="seeDocumentUid('${object.uuid}')" style="background: #10312b" class="custom-button" title="Ver">
                                        <i style="color: white; font-size: 15px" class="fa fa-eye"></i>
                                    </button>
                                    <button onclick="download('${object.uuid}')" class="custom-button" title="Descargar">
                                        <i style="color: white; font-size: 15px" class="fa fa-download"></i>
                                    </button>
                                    <button onclick="openModalOificio('${object.uuid}')" style="background: #6A1B3D" class="custom-button" title="Eliminar">
