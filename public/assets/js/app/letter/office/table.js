...
function searchInit() {
    ...
    $.ajax({
        url: URL_DEFAULT.concat('/office/table'),
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = URL_DEFAULT.concat(`/office/edit/${object.id}`);
                    const finalCloud = URL_DEFAULT.concat(`/office/cloud/${object.id}`);
                    const urlReport = URL_DEFAULT.concat(`/office/generate-pdf/${object.id}`);
