...
function searchInit() {
    ...
    $.ajax({
        url: `${URL_DEFAULT}/coordinacion/table`,
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = `${URL_DEFAULT}/coordinacion/edit/${object.id}`;
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/coordinacion/delete/${id}`,
        type: 'DELETE',
        data: {
            _token: token
        },
        success: function(response) {
            window.location.href = '/srh/public/administration/coordinacionC/list';
        },
