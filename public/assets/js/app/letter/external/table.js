...
function searchInit() {
    ...
    $.ajax({
        url: URL_DEFAULT.concat('/external/table'),
        type: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = URL_DEFAULT.concat(`/external/edit/${object.id}`);
                    const finalCloud = URL_DEFAULT.concat(`/external/cloud/${object.id}`);
