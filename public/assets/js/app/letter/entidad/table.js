        ...
        success: function (response) {
            ...
            if (response.value && response.value.length > 0) {
                response.value.forEach(function (object) {
                    const finalUrl = `${URL_DEFAULT}/entidad/edit/${object.id}`;
...
function deleteCourse(id) {
    $.ajax({
        url: `${URL_DEFAULT}/entidad/delete/${id}`,
        type: 'DELETE',
        data: {
            _token: token
        },
        success: function(response) {
            window.location.href = '/srh/public/administration/entidadC/list';
        },
