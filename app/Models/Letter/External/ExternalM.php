    ...
    public function list($iterator, $searchValue)
{
    $query = DB::table('correspondencia.tbl_circular_externa')
        ->select([
            ...
            DB::raw('correspondencia.cat_dependencia_area.descripcion AS area'), // ✅ NUEVO
            DB::raw('correspondencia.tbl_circular_externa.asunto AS asunto'),
        ])
        ->join(
            'correspondencia.cat_dependencia',
            'correspondencia.tbl_circular_externa.id_cat_dependencia',
            '=',
            'correspondencia.cat_dependencia.id_cat_dependencia'
        )
        ->join( // ✅ join al área (si puede venir null, cámbialo a leftJoin)
            'correspondencia.cat_dependencia_area',
            'correspondencia.tbl_circular_externa.id_cat_dependencia_area',
            '=',
            'correspondencia.cat_dependencia_area.id_cat_dependencia_area'
        );
    ...
    if (!empty($searchValue)) {
        ...
        $query->where(function ($query) use ($searchValue) {
            $query->whereRaw("EXTRACT(YEAR FROM correspondencia.tbl_circular_externa.fecha_documento::DATE)::TEXT LIKE ?", ['%' . $searchValue . '%'])
                ...
                ->orWhereRaw("UPPER(TRIM(correspondencia.cat_dependencia_area.descripcion)) LIKE ?", ['%' . $searchValue . '%']) // ✅ NUEVO
