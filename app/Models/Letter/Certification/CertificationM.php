...
namespace App\Models\Letter\Certification;
...
class CertificationM extends Model
{
    ...
    public function list($iterator, $searchValue)
    {
        ...
        $query = DB::table('correspondencia.tbl_certificaciones')
            ->select([
                'correspondencia.tbl_certificaciones.id_tbl_certificaciones AS id',
                DB::raw("CASE 
                            WHEN correspondencia.tbl_certificaciones.consecutivo_inicio = 
                                    correspondencia.tbl_certificaciones.consecutivo_fin
                            THEN correspondencia.tbl_certificaciones.consecutivo_inicio
                        ELSE 
                            correspondencia.tbl_certificaciones.consecutivo_inicio || ' - ' ||
                            correspondencia.tbl_certificaciones.consecutivo_fin
                        END AS consecutivo"),
                DB::raw("TO_CHAR(correspondencia.tbl_certificaciones.fecha_asignacion, 'DD/MM/YYYY') AS fecha_asignacion"),
                DB::raw("TO_CHAR(correspondencia.tbl_certificaciones.fecha_documento, 'DD/MM/YYYY') AS fecha_documento"),
                DB::raw("LEFT(correspondencia.tbl_certificaciones.asunto, 120) || ' ...' AS asunto"),
                DB::raw("correspondencia.tbl_certificaciones.uuid_pdf AS uuid"),
            ]);
        ...
        if (!empty($searchValue)) {
            ...
            $query->where(function ($query) use ($searchValue) {
                $query->whereRaw("CASE 
                    ...
                    ->orWhereRaw("TO_CHAR(correspondencia.tbl_certificaciones.fecha_asignacion, 'DD/MM/YYYY')::TEXT LIKE ?", ['%' . $searchValue . '%'])
                    ->orWhereRaw("UPPER(TRIM(correspondencia.tbl_certificaciones.asunto)) LIKE ?", ['%' . $searchValue . '%']);
            });
        }

        // Aplicar la paginación (OFFSET y LIMIT)
        $query->orderBy('correspondencia.tbl_certificaciones.id_tbl_certificaciones', 'DESC')
