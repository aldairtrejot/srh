...
{
    public function buscarEmpleadoHraes($valor, $pagina = 1, $limite = 5)
    {
        ...
        return DB::table('central.tbl_empleados_hraes AS e')
            ...
            ->where(function ($query) use ($valor) {
                ...
                $query->whereRaw('UPPER(e.curp) LIKE ?', ["%$valorUpper%"])
                      ->orWhereRaw('UPPER(e.rfc) LIKE ?', ["%$valorUpper%"])
                      ->orWhereRaw('UPPER(e.nombre) LIKE ?', ["%$valorUpper%"])
                      ->orWhereRaw('UPPER(e.primer_apellido) LIKE ?', ["%$valorUpper%"])
                      ->orWhereRaw('UPPER(e.segundo_apellido) LIKE ?', ["%$valorUpper%"]);
            })
            ->orderBy('e.nombre')
            ->offset($offset)
            ->limit($limite)
