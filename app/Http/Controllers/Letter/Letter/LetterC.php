    ...
    try {
        ...
        $corr = DB::table('correspondencia.tbl_correspondencia')
            ->select(
                ...
                'observaciones',
                'folio_gestion',
                'id_cat_estatus'    // ← IMPORTANTE para saber si está CONCLUIDO
            )
        ...
        // ⚠️ SI YA ESTÁ CONCLUIDO (4) → NO PERMITIR MÁS RESPUESTAS
        ...
        // ¿Ya hay oficio para esta correspondencia? (único por constraint)
        ...
        try {
            ...
            if ($oficioExistente) {
                ...
                $ofData = [
                    // num_turno_sistema se conserva (no se cambia)
                ...
                // Para el log agregamos algunos datos claves
                $ofData['id_tbl_oficio']          = $idOficio;
                $ofData['id_tbl_correspondencia'] = $idCorr;
                $ofData['num_turno_sistema']      = $oficioExistente->num_turno_sistema;
            } else {
                // ===== CREAR NUEVO OFICIO (primera respuesta) =====
                $numTurnoOficio = $consec->noDocumento(
                    $corr->id_cat_anio,
            ...
            // Solo FINAL agrega observaciones a la correspondencia
        ...
        // ===== SUBIDAS A ALFRESCO (OFICIO + ANEXOS) =====
        try {
            ...
            if (($hasOficio || count($anexos) > 0) && !empty($idOficio)) {
                ...
                if ($folderId) {
                    // Oficio (FINAL: obligatorio; AVANCE: no se usa en el front)
                    if ($hasOficio) {
                        $file   = $request->file('file_oficio_entrada');
                        $ts     = now()->format('YmdHis');
                    ...
                    // Anexos (AVANCE: obligatorios en el front; FINAL: opcionales)
        ...
        return response()->json([
            ...
            'id_oficio' => $idOficio ?? null,
        ]);
    } catch (\Throwable $e) {
        try {
            if (method_exists(DB::connection(), 'transactionLevel')
