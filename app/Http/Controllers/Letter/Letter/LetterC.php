...
class LetterC extends Controller
{
    ...
    /* ======== HELPERS PRIVADOS PARA RESPUESTAS 422 ======== */
    ...
    public function create()
    {
        ...
        $item->num_turno_sistema  = $collectionConsecutivoM->noDocumento(
            $item->id_cat_anio, config('custom_config.CP_TABLE_CORRESPONDENCIA')
        );
    ...
    public function save(Request $request)
    {
        ...
        try {
            // ===== Validaciones (cadenas y tamaño archivo) =====
            $request->validate([
            ...
            if (!$request->filled('id_tbl_correspondencia')) {
                ...
                // Áreas (mantener NULL si no se elige)
                ...
                $areaFinal = $area3 ?: ($area2 ?: ($area1 ?: null)); // en BD puede quedar NULL
                ...
                \Log::info('[SAVE] CREATE flow — creando tbl_correspondencia', [
                ...
                $collectionConsecutivoM->iteratorConsecutivo(
                    $request->id_cat_anio, config('custom_config.CP_TABLE_CORRESPONDENCIA')
                );
                ...
                // ===== Subir a Alfresco con “área de contexto” si id_cat_area viene NULL =====
            ...
            // Forzar Returnado si alguna área sólo permite Returnado
            try {
                if (
            ...
            if ($hasFullUpdateRole) {
                ...
                // Asegurar área para subir a Alfresco
    ...
    public function collectionArea(Request $request)
    {
        try {
            ...
            if ($scope === 'returnado_flag_by_area') {
                ...
                $onlyReturnado = $m->areaOnlyReturnado($areaId);

                return response()->json([
                    'ok'            => true,
                    'hasReturnado'  => $hasReturnado,
                    'onlyReturnado' => $onlyReturnado,
    ...
    {
        if ($this->isBypassVisibility()) {
            return ['area' => true, 'crh' => true, 'crhtod' => true];
        }
    ...
    /* ===== Guardado local opcional (espejo) ===== */
                ...
                if ($hasOficio || count($anexos) > 0) {
                    ...
                    if ($folderId) {
                        ...
                        if ($hasOficio) {
                            ...
                            $customName = "OFICIO_{$folioSafe}_{$ts}R.{$ext}"; // R de Respuesta
