...
log::info($initials);
        ...
        try {
            ...
            if (false) {
                ...
                if ($anioActual !== null && $anioActual !== '') {
                    ...
                    if ($existsA || $existsB) {
                        return $this->respondValidation422($request, ['folio_gestion' => ['123 El folio de gestión ya existe.']]);
                    }
                } else {
                    $exists = $q->when($idActual, fn ($qq) => $qq->where('id_tbl_correspondencia', '<>', $idActual))->exists();
                    if ($exists) {
                        return $this->respondValidation422($request, ['folio_gestion' => ['124 El folio de gestión ya existe.']]);
                    }
                }
            }
            ...
            if (! $request->filled('id_tbl_correspondencia')) {
                ...
                try {
                    ...
                    if ((string) $qe->getCode() === '23505') {
                        ...
                        return $this->respondValidation422($request, ['folio_gestion' => ['1Ya existe un registro con estos datos.']]);
                    }
                    throw $qe;
                    */
                }
            ...
            if ($hasFullUpdateRole) {
                ...
                try {
                    ...
                    if ((string) $qe->getCode() === '23505') {
                        ...
                        return $this->respondValidation422($request, ['folio_gestion' => ['2 Ya existe un registro con estos datos.']]);
                    }
                    throw $qe;
                }
            ...
            if ($e instanceof \Illuminate\Database\QueryException && (string) $e->getCode() === '23505') {
                ...
                return $this->respondValidation422($request, ['folio_gestion' => ['Y3 a existe un registro con estos datos.']]);
            }
    ...
    public function collectionArea(Request $request)
    {
        try {
            ...
            if ($scope === 'dependents_by_area3') {
                ...
                if (! $a3) {
                    return response()->json(['ok' => false, 'message' => 'id_cat_area requerido'], 422);
                }
            ...
            if ($scope === 'dependents_by_area2') {
                ...
                if (! $a2) {
                    return response()->json(['ok' => false, 'message' => 'id_cat_area_2 requerido'], 422);
                }
            ...
            if ($scope === 'dependents_by_area1') {
                ...
                if (! $a1) {
                    return response()->json(['ok' => false, 'message' => 'id_cat_area_1 requerido'], 422);
                }
        ...
        try {
            ...
            switch ($type) {
                case 'folio':
                    ...
                    if ($id) {
                        $q->where('id_tbl_correspondencia', '<>', $id);
                    }
                    $existsA = $q->exists();
                    $existsB = false;
                    if ($qA) {
                        if ($id) {
                            $qA->where('id_tbl_correspondencia', '<>', $id);
                        } $existsB = $qA->exists();
                    }
    ...
    private function mapUniqueErrorToField(\Illuminate\Database\QueryException $e): ?array
    {
        ...
        if (stripos($msg, 'tbl_correspondencia_folio_gestion_key') !== false || stripos($msg, '(folio_gestion)') !== false) {
            return ['folio_gestion' => ['12 El folio de gestión ya existe.']];
        }
    ...
    public function normalizeFolderId(?string $value): ?string
    {
        ...
        if (preg_match('#workspace://SpacesStore/([0-9a-fA-F-]{36})#', $value, $m)) {
            return $m[1];
        }
        ...
        if (preg_match('/([0-9a-fA-F-]{36})/', $value, $m3)) {
            return $m3[1];
        }
    ...
    {
        if (preg_match('/\/([0-9]{5})\//', (string) $numTurno, $m)) {
            return (int) $m[1];
        }
