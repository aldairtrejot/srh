...
public function save(Request $request)
{
    ...
    try {
        ...
        /* ==================== PRE-VALIDACIÓN DE UNICIDAD (robusta) ==================== */
        // Nota: replicamos lógica típica de índices únicos (TRIM+UPPER) y contemplamos variante compuesta con id_cat_anio
        ...
        if ($folioClean !== '') {
            ...
            // si existe columna/índice por año, esto cubre ambos escenarios (con y sin anio)
            if ($anioActual !== null && $anioActual !== '') {
                $queryAnio = (clone $query)->where('id_cat_anio', $anioActual);
                $existsA   = $query->when($idActual, fn($q)=>$q->where('id_tbl_correspondencia','<>',$idActual))->exists();
                $existsB   = $queryAnio->when($idActual, fn($q)=>$q->where('id_tbl_correspondencia','<>',$idActual))->exists();
                if ($existsA || $existsB) {
                ...
                $exists = $query->when($idActual, fn($q)=>$q->where('id_tbl_correspondencia','<>',$idActual))->exists();
                if ($exists) {
                    return $this->respondValidation422($request, [
                        'folio_gestion' => ['El folio de gestión ya existe.']
                    ]);
        ...
        if (!$request->filled('id_tbl_correspondencia')) {
            ...
            // ======== TRANSACCIÓN: insert + logs + iterator. Subidas DESPUÉS del commit. ========
            DB::beginTransaction();
            try {
                /** @var \App\Models\Letter\Letter\LetterM $created */
                ...
                // ===== Subidas (sólo tras commit exitoso) =====
                ...
                DB::rollBack();
                // Si falló por UNIQUE, mapeamos a 422 (sin subir archivos)
...
public function validateUnique(Request $request)
{
    try {
        ...
        $id        = $request->input('id'); // id_tbl_correspondencia (para excluir en edición)
        ...
        switch ($type) {
            case 'folio': {
                ...
                // contempla índice por año si aplica
                $qAnio = $anio ? (clone $q)->where('id_cat_anio', $anio) : null;
                ...
                $existsB = false;
                if ($qAnio) {
                    if ($id) { $qAnio->where('id_tbl_correspondencia', '<>', $id); }
                    $existsB = $qAnio->exists();
                }
...
public function uploadFilesIfAny(Request $request, int $idCorrespondencia): void
{
    try {
        ...
        $hexUrl = env('ALFRESCO_URL_ADD') !== null
            ? implode('', array_map(fn($c) => strtoupper(str_pad(dechex(ord($c)), 2, '0', STR_PAD_LEFT)), str_split(env('ALFRESCO_URL_ADD'))))
            : '';
        ...
        $rawFolder = optional($cloudConfigM->getUid(
            $request->id_cat_area,
        ...
        if (!$rawFolder) {
            $fallback = DB::table('correspondencia.cat_config_cloud')
            ...
            if ($fallback) {
                $rawFolder = $fallback;
                \Log::info('[UPLOAD] usando fallback de carpeta por área', [
                    'area' => $request->id_cat_area,
                    'uid'  => $rawFolder
                ]);
            }
